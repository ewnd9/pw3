...
module.exports = function (config) {
  ...
  var q2 = {
    ...
    choices: _.map(programs, function (program) {
      var name = [program.name, pathUtils.isInPath(program.name) ? '(installed)' : '(not founded)', // Inquirer's disabled doesn't seem to work for me
      '[' + program.description + ']', '[' + program.script + ']'];
